...
private fun RecipeSearchBar(
    searchQuery: String,
    suggestions: List<RecipeListItem>,
    ...
    LaunchedEffect(listState) {
        ...
        snapshotFlow { listState.firstVisibleItemIndex to listState.firstVisibleItemScrollOffset }
            .collect { (index, offset) ->
                ...
                showBar = !scrollingDown
                prevIndex = index
                prevOffset = offset
            }
    }
    LaunchedEffect(searchQuery) { if (searchQuery != localQuery) localQuery = searchQuery }

    AnimatedVisibility(
        visible = showBar || searchActive.value,
        enter = fadeIn(tween(180)) + slideInVertically(tween(180)) { -it },
        exit = fadeOut(tween(180)) + slideOutVertically(tween(180)) { -it }
    ) {
        SearchBar(
            query = localQuery,
            onQueryChange = { localQuery = it; onQueryChange(it) },
            onSearch = { q -> onQueryChange(q); searchActive.value = false },
            ...
            trailingIcon = {
                if (searchQuery.isNotBlank()) {
                    IconButton(onClick = { onClearQuery(); localQuery = "" }) {
            ...
            LazyColumn(modifier = Modifier.heightIn(max = 600.dp)) {
                items(suggestions) { r ->
                    Card(modifier = Modifier.fillMaxWidth()
                        .clickable {
                            try { onSuggestionClick(r.id, r.title) } catch (e: Exception) { Timber.w(e) }
