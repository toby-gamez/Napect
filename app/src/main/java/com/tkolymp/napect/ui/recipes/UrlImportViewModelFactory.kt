...
class UrlImportViewModelFactory(private val service: UrlImportService, private val repo: RecipeRepository, private val gemini: GeminiNanoService? = null) : ViewModelProvider.Factory {
    override fun <T : ViewModel> create(modelClass: Class<T>): T {
        if (modelClass.isAssignableFrom(UrlImportViewModel::class.java)) {
            @Suppress("UNCHECKED_CAST")
            return UrlImportViewModel(service, repo, gemini) as T
        }
